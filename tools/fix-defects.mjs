...
for (const song of raw) {
  ...
  for (let pass = 0; pass < budget; pass++) {
    ...
    if (!acted && !song.hasAuthoredF) {
      const note = d.h === 'L' ? d.hi : d.lo;      // the outlier: top of a left hand, bottom of a right
      const to = d.h === 'L' ? 'R' : 'L';
      ...
      if (!dup && holdable(pitches, to)) {
        const was = note.h;
        note.h = to;
        if (defects(work, bpm).length < bad.length) { moves++; acted = true; }
        else note.h = was;
      }
    }
