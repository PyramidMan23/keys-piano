...
import { writeMidi } from '../../keys-piano/tools/midi.mjs';
...
if (!eventsPath || !templatePath || !bpm) { console.error('usage: to-import.mjs <events.json> <template.json> --bpm N [--meter 4] --out song.mid'); process.exit(2); }
...
console.log(`${ev.length} events at ${bpm}bpm, grid phase ${(bestPhase * 1000).toFixed(0)}ms: mean move ${mean.toFixed(3)} beats, worst ${worst.toFixed(3)}`);
