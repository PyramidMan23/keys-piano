...
- **Dark only, and that is deliberate.** Checked in the running app: `style.css` declares
  `color-scheme: dark` and ships exactly one palette. He plays at night, at the piano, in a dark room.
  Do not design a light theme and do not add a theme toggle.
